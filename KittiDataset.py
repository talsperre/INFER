...
class KittiDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.channels is None:
            self.channels = ["lane", "obstacles", "road", "vehicles"]
        
        for chan in self.channels:
            if chan == "lane":
                inpTensor = torch.cat((inpTensor, curLaneTensor), dim=0)
            if chan == "obstacles":
                inpTensor = torch.cat((inpTensor, curObstacleTensor), dim=0)
            if chan == "road":
                inpTensor = torch.cat((inpTensor, curRoadTensor), dim=0)
            if chan == "vehicles":
                inpTensor = torch.cat((inpTensor, curVehiclesTensor), dim=0)
